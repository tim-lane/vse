...
public class TcpStubWorker extends StubWorker implements Runnable{
  ...
  public void run() {
    ...
    try
    {
      ...
      try {
// tcp assumes single line for the moment, assumes enter is end of line ch = 10
        inStream = new TcpInputStream(clientSocket.getInputStream());
        inputMsgLines.addElement(inStream.readLine());
        
      } catch (Exception e) {
        date = new Date();
        formattedDate = sdf.format(date);
        logger.error("tcpStubWorker: " + formattedDate + " error in getting header : " + e);
        e.printStackTrace();
      }
      if (logger.isDebugEnabled()) {
        logger.debug("tcpStubWorker: inputMessage : " + inputMsgLines.toString());
      }
      /* 
      ...
      messageType=message.getMessageType(); // cdef
      
      
      if (messageType.equals("CDEF")){
        if (logger.isDebugEnabled()) {
          logger.debug("tcpStubWorker: processing CDEF message");
        }
        // any cdef pre processing here //
      }
